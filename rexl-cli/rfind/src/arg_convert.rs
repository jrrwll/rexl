...
pub fn convert_args<'a>(parser: &ArgParser<Options>, context: &'a Context) -> Main<'a> {
    ...
    let name_pattern = parser.get_strings_or_empty(NamePattern).iter().map(|s| {
        match Regex::from_str(s) {
            Ok(v) => { v }
            Err(err) => {
                eprintln!("{}", context.format(
                    "name-pattern.unknown-format", vec![s.to_string(), err.to_string()]));
                process::exit(7);
            }
        }
    }).collect();
    ...
    let size = if let Some(s) = parser.get_string(Size) {
        SizeOption::parse(s).iter().map(|opt| {
            if let Some(su) = opt {
                *su
            } else {
                eprintln!("{}", context.format("size.unknown-format", vec![s.to_string()]));
                process::exit(8);
            }
    ...
    let content_pattern = parser.get_strings_or_empty(ContentPattern).iter().map(|s| {
        match Regex::from_str(s) {
            Ok(v) => { v }
            Err(err) => {
                eprintln!("{}", context.format(
                    "content-pattern.unknown-format", vec![s.to_string(), err.to_string()]));
                process::exit(9);
            }
        }
    }).collect();
