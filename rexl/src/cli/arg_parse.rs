...
pub struct ArgParser<K: Hash + Eq + Debug + Clone> {
    ...
    pub names_key_map: HashMap<String, K>,
    /// key, argument
    pub key_argument_map: HashMap<K, Argument<K>>,
    /// key, value
    pub string_map: HashMap<K, String>,
    pub bool_map: HashMap<K, bool>,
    pub integer_map: HashMap<K, i64>,
    pub float_map: HashMap<K, f64>,
    /// multiple values
    pub strings_map: HashMap<K, Vec<String>>,
    pub integers_map: HashMap<K, Vec<i64>>,
    pub floats_map: HashMap<K, Vec<f64>>,
    pub properties_map: HashMap<K, HashMap<String, String>>,
    /// extra values
    pub extra_values: Vec<String>,
}

impl<K> ArgParser<K> where K: Hash + Eq + Debug + Clone {
    ...
    pub fn get_string_or_else<T, F>(&self, key: T, f: F) -> String
        where T: Into<K>, F: FnOnce() -> String {
    ...
    pub fn get_strings_or_else<T, F>(&self, key: T, f: F) -> Vec<String>
        where T: Into<K>, F: FnOnce() -> Vec<String> {
    ...
    pub fn get_strings_or_empty<T>(&self, key: T) -> Vec<String>
        where T: Into<K>{
        self.get_strings_or_else(key, #[inline] || Vec::new())
    }
    ...
    pub fn get_integers_or_else<T, F>(&self, key: T, f: F) -> Vec<i64>
        where T: Into<K>, F: FnOnce() -> Vec<i64> {
    ...
    pub fn get_integers_or_empty<T>(&self, key: T) -> Vec<i64>
        where T: Into<K>{
        self.get_integers_or_else(key, #[inline] || Vec::new() )
    }
    ...
    pub fn get_floats_or_else<T, F>(&self, key: T, f: F) -> Vec<f64>
        where T: Into<K>, F: FnOnce() -> Vec<f64>{
    ...
    pub fn get_floats_or_empty<T>(&self, key: T) -> Vec<f64>
        where T: Into<K> {
        self.get_floats_or_else(key, #[inline] || Vec::new() )
    }
    ...
    pub fn get_properties_or_else<T, F>(&self, key: T, f: F) -> HashMap<String, String>
        where T: Into<K>, F: FnOnce() -> HashMap<String, String> {
    ...
    pub fn get_properties_or_empty<T>(&self, key: T) -> HashMap<String, String>
        where T: Into<K> {
        self.get_properties_or_else(key, #[inline] || HashMap::new() )
    }
    ...
    pub fn add_by_kind<T>(&mut self, key: T, names: Vec<&str>,
                          kind: ArgumentKind, multiple: bool) -> &mut Self where T: Into<K> {
        let key = key.into();
        let names = names.iter().map(|s| s.to_string()).collect();
        self.key_argument_map.insert(key.clone(), Argument {
            key, names, kind, multiple
        });
        return self
    }
}